...
public sealed class ProductService(CloneEbayDbContext context) : IProductService
{
    public async Task<ApiResponse<PagedResultDto<ProductListItemDto>>> GetProductsAsync(string? search, int? categoryId, decimal? minPrice, decimal? maxPrice, int page, int pageSize)
    {
        ...
        var query = context.Products.AsNoTracking().Include(x => x.Category).Include(x => x.Reviews).AsQueryable();
    ...
    {
        var product = await context.Products.AsNoTracking().Include(x => x.Category).Include(x => x.Seller).Include(x => x.Reviews).ThenInclude(x => x.Reviewer).FirstOrDefaultAsync(x => x.Id == id);
